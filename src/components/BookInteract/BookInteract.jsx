...
function BookInteract({ book, mode }) {
    ...
    const update = async (id) => {
        await axios.put(`https://node56983-chirawat-noderest.proen.app.ruk-com.cloud/books/${id}`, payload)
            .then((resp) => {
                if (resp.status == 200) {
    ...
    const add = async () => {
        await axios.post('https://node56983-chirawat-noderest.proen.app.ruk-com.cloud/books', payload)
            .then((resp) => {
                if (resp.status == 200) {
                    window.location.reload();
